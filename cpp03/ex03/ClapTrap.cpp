...
{
    std::cout << "ClapTrap empty constructor init called" << std::endl;
}

ClapTrap::ClapTrap(std::string name) : _name(name), _hits(10), _energy(10), _attackDamage(0)
{
    std::cout << "ClapTrap Constructor init with string as parameter" << std::endl;
}
ClapTrap::ClapTrap(const ClapTrap &c)
{
    std::cout << "ClapTrap Constructor init with ClapTrap reference" << std::endl;
    *this = c;
}
ClapTrap &ClapTrap::operator=(const ClapTrap &rhs)
{
    std::cout << "ClapTrap constructor init by assignation" << std::endl;
    if (this != &rhs)
    {
