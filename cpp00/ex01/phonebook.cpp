...
void Phonebook::add(void)
{
    ...
    std::cout << std::endl;
    this->_contact[this->_id++ % MAX_CONTACTS] = Contact(firstName, lastName, nickname, phoneNumber, darksetSecret);
}
...
void Phonebook::_displayGrid(void)
{
    ...
    {
        this->_printField(std::to_string(i));
...
void Phonebook::_displayContact(std::string query)
{
    ...
    try
    {
        index = std::stoi(query);
        if (index > MAX_CONTACTS || index >= this->_id)
