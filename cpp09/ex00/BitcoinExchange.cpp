...
void BitcoinExchange::displayLineConversion(t_data &d) const {
  if (!d.isValidInput || d.timestamp == 0)
    throw InvalidInput();
  else if (d.quantity < 0)
    throw NotPositiveNumber();
  else if (d.quantity >= 1000)
    throw TooLargeQuantity();
  else if (d.nearestPrice < 0)
    throw BtcDidNotExist();
  else {
    std::cout << d.date << "=> " << d.quantity << " = "
              << d.nearestPrice * d.quantity << std::endl;
  }
}
...
void BitcoinExchange::showConversions() {
  ...
  for (it = this->_values.begin(); it != this->_values.end(); it++) {
    ...
    } catch (const BitcoinExchange::BtcDidNotExist &e) {
      std::cerr << "Error: " << e.what() << line.date << std::endl;
...
float BitcoinExchange::getNearestPrice(t_data &req) const {
  ...
  if (db.is_open()) {
    ...
    while (std::getline(db, line)) {
      ...
      if (tm > req.timestamp)
        break;
      if (!target.isValidInput ||
          req.timestamp - tm < req.timestamp - target.timestamp) {
        target.isValidInput = true;
        target.timestamp = tm;
        target.date = date;
        target.nearestPrice =
            static_cast<float>(atof(line.substr(pos + 1).c_str()));
      }
    }
  }
  db.close();
